	...
	$SCRIPT_ARCHITECTURE_AVR)
		...
		case $SCRIPT_SCENARIO in
			...
			$SCRIPT_SCENARIO_1)
				...
				if [ -f $avr_execution_time_log_file ] ; then
					eks_execution_time=$(compute_execution_time $avr_execution_time_log_file 'RunEncryptionKeySchedule' 'EndEncryptionKeySchedule')
					e_execution_time=$(compute_execution_time $avr_execution_time_log_file 'EncryptScenario1' 'EndEncryption')
					dks_execution_time=$(compute_execution_time $avr_execution_time_log_file 'RunDecryptionKeySchedule' 'EndDecryptionKeySchedule')
					d_execution_time=$(compute_execution_time $avr_execution_time_log_file 'DecryptScenario1' 'EndDecryption')
				fi
				;;
			$SCRIPT_SCENARIO_2)
				simulate $file $avr_execution_time_log_file
				...
				if [ -f $avr_execution_time_log_file ] ; then
					e_execution_time=$(compute_execution_time $avr_execution_time_log_file 'EncryptScenario2' 'EndEncryption')
				fi
				;;
		esac
...
if [ $SCRIPT_MODE_0 -eq $SCRIPT_MODE ] ; then
	...
	case $SCRIPT_SCENARIO in
		...
		$SCRIPT_SCENARIO_2)
			# Display results
			printf "%s" $e_execution_time > $SCRIPT_OUTPUT
			;;
	esac
fi
