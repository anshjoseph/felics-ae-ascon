...
function add_csv_table_header()
{
	...
	case $scenario in
		$SCRIPT_SCENARIO_0)
			...
			printf "\n" >> $output_file
			printf "Cipher,State Size (bits),Key Size (bits),IV (bits),Version,Language,Options,S (bytes),E (bytes),S + E (bytes),S (bytes),E (bytes),S (bytes),E (bytes),Common (bytes),Total (bytes),S (cycles),E (cycles)" >> $output_file
			printf "\n" >> $output_file
			;;
		$SCRIPT_SCENARIO_1)
			printf "Cipher Info,,,,Implementation Info,,,Code Size,,,RAM,,,,,,Execution Time," >> $output_file
