...
typedef uint8_t block[16];
 ...
 void xor_block(block d, const block s1, const block s2) {
    uint32_t i;
    for (i=0; i<16; i++)
        d[i] = s1[i] ^ s2[i];
}
...
/* ------------------------------------------------------------------------- */
 ...
 void double_block(block d, const block s) {
    int32_t i;
    uint8_t tmp = s[0];
    for (i=0; i<15; i++)
        d[i] = (s[i] << 1) | (s[i+1] >> 7);
    uint8_t tmpb = tmp;
    d[15] = ((tmp >> 7) * 130) + (tmpb >> 7) + (tmpb >> 7) + (tmpb >> 7) + (tmpb >> 7) + (tmpb >> 7);
    //d[15] = ((tmp >> 7) * 8);
    d[15] ^= (s[15] << 1);
}
...
/* ------------------------------------------------------------------------- */
 ...
 void calc_L_i(block l, const block ldollar, int32_t i) {
...
static int ocb_crypt( uint8_t *out, const uint8_t*k, const uint8_t *n,
...
{
	*clen = mlen + TAGBYTES;
    ocb_crypt(c, k, npub, ad,
            adlen, m, mlen, OCB_ENCRYPT, roundKeys);
    return 0;
}
