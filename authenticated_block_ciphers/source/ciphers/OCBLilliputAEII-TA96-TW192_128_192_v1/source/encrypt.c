...
void subkey(const uint8_t key[KEY_SIZE], const uint8_t tweak[TWEAK_SIZE], uint8_t *roundkeys){
	...
	memcpy(TK1, key, 16);
	memcpy(TK2, key+16, 8);
	memcpy(TK2+8, tweak, 8);
	memcpy(TK3, tweak+8, 16);
	...
	for(i=0; i<59; i++){
		...
		for(j = 0 ; j < 8 ; j++){
			...
			TK3[j] = (TK3[j] >>1) | ( ( TK3[j] <<7)^((TK3[j] <<1) & 0x80) ) ;
		}
	}
...
void update_tweak(uint8_t *tweak, const uint8_t value, const uint64_t counter) {
    ...
    tweak[ 16] = ((counter >> 56ULL) & 0xff);
    tweak[ 17] = ((  counter >> 48ULL) & 0xff);
    tweak[ 18] = ((  counter >> 40ULL) & 0xff);
    tweak[ 19] = (((  counter >> 31ULL) >>1) & 0xff);
    tweak[ 20] = ((  counter >> 24ULL) & 0xff);
    tweak[ 21] = ((  counter >> 16ULL) & 0xff);
    tweak[ 22] = ((  counter >>  8ULL) & 0xff);
    tweak[ 23] = ((  counter >>  0ULL) & 0xff);
}

void uptdate_tag_in_tweak(uint8_t *temp_tweak, const uint8_t *tweak, const uint64_t counter) {
    temp_tweak[ 16] = tweak[ 16] ^ ((counter>>56) & 0xff );
    temp_tweak[ 17] = tweak[ 17] ^ ((counter>>48) & 0xff );
    temp_tweak[18] = tweak[18] ^ ((counter>>40) & 0xff );
    temp_tweak[19] = tweak[19] ^ ((counter>>32) & 0xff );
    temp_tweak[20] = tweak[20] ^ ((counter>>24) & 0xff );
    temp_tweak[21] = tweak[21] ^ ((counter>>16) & 0xff );
    temp_tweak[22] = tweak[22] ^ ((counter>> 8) & 0xff );
    temp_tweak[23] = tweak[23] ^ ((counter>> 0) & 0xff );
}
