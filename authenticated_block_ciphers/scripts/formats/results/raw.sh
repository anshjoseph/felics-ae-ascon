...
function add_raw_table_header()
{
	...
	case $scenario in
		...
		$SCRIPT_SCENARIO_3)
			# Table title
			title_position=$(($SCENARIO_3_TABLE_HORIZONTAL_LINE_LENGTH / 2 + 15))
			printf " %"$title_position"s " "Architecture: $architecture; Scenario: $scenario" >> $output_file
			printf "\n" >> $output_file

			# Table header
			printf "%0.s-" $(seq 1 $SCENARIO_3_TABLE_HORIZONTAL_LINE_LENGTH) >> $output_file
			printf "\n" >> $output_file
			printf "| %39s | %39s | %67s | %137s | %53s |\n" "Cipher Info" "Implementation Info" "Code Size" "RAM" "Execution Time" >> $output_file
			printf "%0.s-" $(seq 1 $SCENARIO_3_TABLE_HORIZONTAL_LINE_LENGTH) >> $output_file
			printf "\n" >> $output_file
...
function add_raw_table_footer()
{
	...
	case $scenario in
		...
		$SCRIPT_SCENARIO_2)
			...
			;;
		$SCRIPT_SCENARIO_3)
			horizontal_line_length=$SCENARIO_3_TABLE_HORIZONTAL_LINE_LENGTH
			;;
	esac
