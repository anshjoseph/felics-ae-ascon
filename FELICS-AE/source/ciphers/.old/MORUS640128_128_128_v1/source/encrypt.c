...
void morus_stateupdate(uint32_t msgblk[], uint32_t state[][4])
{
	uint32_t temp;
...
void morus_initialization(const uint8_t *key, const uint8_t *iv, uint32_t state[][4])
{
	...
	for (i = 0; i < 4; i++) state[1][i] ^= ((uint32_t*)(void*)key)[i];
}

//the finalization state of MORUS
void morus_tag_generation(int32_t msglen, int32_t adlen, uint8_t *c, uint32_t state[][4])
{
	int32_t i, j;
	uint8_t t[16];
	for(i =0; i<16; i++) t[i] = 0;

	((int32_t*)(void*)t)[0] = (adlen << 3);
	((int32_t*)(void*)t)[1] = (msglen << 3);
	
	//memcpy(tab0, state[0], 4);
	...
	//memcpy(tab0, c + msglen, 16);
}

// one step of encryption: it encrypts a 16-byte block 
void morus_enc_aut_step(const uint8_t *plaintextblock, uint8_t *ciphertextblock, uint32_t state[][4])
{

	//encryption   
	((uint32_t*)(void*)ciphertextblock)[0] = ((uint32_t*)(void*)plaintextblock)[0] ^ state[0][0] ^ state[1][1] ^ (state[2][0] & state[3][0]);
	((uint32_t*)(void*)ciphertextblock)[1] = ((uint32_t*)(void*)plaintextblock)[1] ^ state[0][1] ^ state[1][2] ^ (state[2][1] & state[3][1]);
	((uint32_t*)(void*)ciphertextblock)[2] = ((uint32_t*)(void*)plaintextblock)[2] ^ state[0][2] ^ state[1][3] ^ (state[2][2] & state[3][2]);
	((uint32_t*)(void*)ciphertextblock)[3] = ((uint32_t*)(void*)plaintextblock)[3] ^ state[0][3] ^ state[1][0] ^ (state[2][3] & state[3][3]);

	morus_stateupdate(((uint32_t*)(void*)plaintextblock), state);
}

// encrypt a partial block  
void morus_enc_aut_partialblock(const uint8_t *plaintext, uint8_t *ciphertext, int32_t len, uint32_t state[][4])
{
	...
	)
{
	...
	morus_tag_generation(mlen, adlen, c + mlen, morus_state);
