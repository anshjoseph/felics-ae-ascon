...
time ./get_results.sh "$architectures" "$scenarios" "$ciphers" "$compiler_options" "$format" "$prefix" >> $BLOCK_CIPHERS_GET_RESULTS_LOG_FILE
...
time ./get_results.sh "$architectures" "$scenarios" "$ciphers" "$compiler_options" "$format" "$prefix" >> $STREAM_CIPHERS_GET_RESULTS_LOG_FILE
