...
# 	./cipher_ram.sh [{-h|--help}] [--version] [{-m|--mode}=[0|1]] [{-s|--scenario}=[0|1]] [{-a|--architecture}=[PC|AVR|MSP|ARM]] [{-t|--target}=[...]] [{-o|--output}=[...]] [{-b|build}=[0|1]] [{-co|--compiler_options}='...']
#
...
#		./cipher_ram.sh -t=./../../source/ciphers/CipherName_StateSizeInBits_KeySizeInBits_IVSizeInBits_v01/build
#
...
function simulate()
{
	...
	case $SCRIPT_ARCHITECTURE in
		...
		$SCRIPT_ARCHITECTURE_ARM)
			...
			$JLINK_GDB_SERVER &> $simulator_output_file &
...
# Get the state, key, IV size
key_size=$(cat $CONSTANTS_SOURCE_FILE | grep "$KEY_SIZE_DEFINE" | tr -d '\r' | cut -d ' ' -f 3)
iv_size=$(cat $CONSTANTS_SOURCE_FILE | grep "$IV_SIZE_DEFINE" | tr -d '\r' | cut -d ' ' -f 3)
...
do
	...
	do
		...
		case $constants_section in
			$CONSTANTS_SECTION_S)
				shared_constants_s=$(($shared_constants_s + $shared_value))
				;;
			$CONSTANTS_SECTION_E)
...
case $SCRIPT_SCENARIO in
	$SCRIPT_SCENARIO_0)
		data_ram_s=$shared_constants_s
		data_ram_e=$shared_constants_e

		stream_size=$(cat $CONSTANTS_SOURCE_FILE | grep "$TEST_STREAM_SIZE_DEFINE" | tr -d '\r' | cut -d ' ' -f 3)
		# Data RAM = key + IV + state + stream + shared constants
		data_ram_common=$(($key_size + $iv_size + $state_size + $stream_size))
	...
	$SCRIPT_SCENARIO_1)
		...
		# Data RAM = key + IV + state + data/stream + shared constants
		data_ram_common=$(($key_size + $iv_size + $state_size + $data_size))
...
if [ -f $gdb_stack_sections_log_file ] ; then
	case $SCRIPT_SCENARIO in
		$SCRIPT_SCENARIO_0)
			s_stack=$(compute_stack_usage $gdb_stack_sections_log_file 1)
...
if [ $SCRIPT_MODE_0 -eq $SCRIPT_MODE ] ; then
	...
	printf "\n" >> $SCRIPT_OUTPUT
	printf "| %10s | %10s | %10s | %10s | %10s | %10s | %10s |\n" "State Size" "Key Size" "IV Size" "Data RAM" "Scenario" "Setup" "Enc." >> $SCRIPT_OUTPUT
	printf "%0.s-" $(seq 1 $TABLE_HORIZONTAL_LINE_LENGTH) >> $SCRIPT_OUTPUT
	printf "\n" >> $SCRIPT_OUTPUT

	# Table line
	printf "| %10s | %10s | %10s | %10s | %10s | %10s | %10s |\n" $state_size $key_size $iv_size $data_ram_total $total_stack $s_stack $e_stack >> $SCRIPT_OUTPUT
	...
	case $SCRIPT_SCENARIO in
		$SCRIPT_SCENARIO_0)
			# Display results
			printf "%s %s %s %s %s %s" $s_stack $e_stack $data_ram_s $data_ram_e $data_ram_common $data_ram_total > $SCRIPT_OUTPUT
			;;
		$SCRIPT_SCENARIO_1)
			# Display results
			printf "%s %s %s %s %s %s" $s_stack $e_stack $data_ram_s $data_ram_e $data_ram_common $data_ram_total > $SCRIPT_OUTPUT
